...
export class QRCode extends React.Component<IProps, {}> {
    ...
    private drawPositioningPattern(
        ...
        // Outer box
        this.drawRoundedSquare(lineWidth, x, y, size, colorOuter, radiiOuter as number | number[], false, ctx);

        // Inner box
        size = cellSize * 3;
        y += cellSize * 2;
        x += cellSize * 2;
        this.drawRoundedSquare(lineWidth, x, y, size, colorInner, radiiInner as number | number[], true, ctx);
    };
    ...
    update() {
        ...
        const quietZone = +(this.props.quietZone || 10);
        ...
        const qrCode = qrGenerator(0, ecLevel || 'M');
        ...
        ctx.strokeStyle = fgColor || '#000000';
        if (qrStyle === 'dots') {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            this.drawPositioningPattern(ctx, cellSize, offset, row, col, color || fgColor || '#000000', radii as CornerRadii);
        }
    ...
    render() {
        const qrSize = +(this.props.size || 150) + (2 * +(this.props.quietZone || 10));
